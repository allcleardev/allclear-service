...
public class TestsResourceTest
{
	...
	public void add()
	{
		...
		var response = request()
			.post(Entity.entity(VALUE = new TestsValue(PERSON.id, FIRST.id, TAKEN_ON, FACILITY.id, true, "All the other details"), UTF8MediaType.APPLICATION_JSON_TYPE));
		Assertions.assertEquals(HTTP_STATUS_OK, response.getStatus(), "Status");

		var value = response.readEntity(TestsValue.class);
		Assertions.assertNotNull(value, "Exists");
		check(VALUE.withId(1L).withPersonName(PERSON.name).withType(FIRST).withFacilityName(FACILITY.name).withCreatedAt(now).withUpdatedAt(now), value);
	}
