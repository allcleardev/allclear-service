...
public abstract class AbstractRedisClientTest
{
	...
	{
		MAP.put("07030", "Hoboken");
		MAP.put("90210", "Beverly Hills");
		MAP.put("10012", "New York");
	}
	...
	public void test_11_listQueue()
	{
		...
		List<String> values = client.list("queue:auditLog");
	...
	{
		List<String> values = client.list("queue:auditLog", 1, 1);
	...
	@Test
	public void test_15_readd()
	{
		test_10_push();
	}

	@Test
	public void test_15_unqueue()
	{
		Assert.assertEquals("Size", 3, client.queueSize("queue:auditLog"));

		client.unqueue("queue:auditLog", "request 2");
		client.unqueue("queue:auditLog", "Request 1");	// Request 1 (not request 1) does not exist.

		Assert.assertEquals("Size", 2, client.queueSize("queue:auditLog"));
	}

	@Test
	public void test_15_unqueue_check()
	{
		Assert.assertEquals("Size", 2, client.queueSize("queue:auditLog"));

		Assert.assertEquals("Check pop", "request 1", client.pop("queue:auditLog"));
		Assert.assertEquals("Check pop", "request 3", client.pop("queue:auditLog"));
	}
	...
	{
		Set<String> values = client.set("set:values");
	...
	{
		Map<String, String> values = client.hash("hash:values");
		Assert.assertEquals("Size", 10, values.size());
